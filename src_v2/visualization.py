...
class HoughTuningBrowser:
    ...
    def _render(self, res: PipelineResult):
        ...
        self._fig.suptitle(
            ...
            f"mOff={c['MASK_OTSU_OFFSET']} mArea={c['MASK_MIN_COMPONENT_AREA']} "
            f"fg={c['WATERSHED_DT_FG_RATIO']:.2f} circ={c['GEOM_MIN_CIRCULARITY']:.2f} "
            f"hGap={c['HYBRID_MASK_OVER_HOUGH_MIN_GAP']} hMin={c['HYBRID_MASK_OVER_HOUGH_MIN_HOUGH_COUNT']} | "
        ...
        if self._status_text is not None:
            self._status_text.set_text(
                (
                    ...
                    f"Zoom: mouse wheel | reset zoom: right click | sliders tune Hough+mask/hybrid"
                )
            )
