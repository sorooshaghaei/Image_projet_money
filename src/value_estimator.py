...
def gold_score_from_inner_lab(l: float, a: float, b: float) -> float:
    ...
    low_red_term = 1.0 - clamp_value((a_shift - 20.0) / 25.0, 0.0, 1.0)
    return float(np.clip(0.55 * yellow_term + 0.25 * bright_term + 0.20 * low_red_term, 0.0, 1.0))
...
def draw_and_analyze_circle_inner_border_colors(
    ...
    if material_mode not in ("hsv", "lab"):
        raise ValueError("material_mode must be one of: 'hsv', 'lab'")
...
class ValueEstimator:
    ...
    def draw_coin_value_labels(
        ...
        for row in rows:
            ...
            label = f"#{coin_id + 1} {pred['best_label']} {int(round(pred['best_prob'] * 100.0)):02d}%"
            font = cv2.FONT_HERSHEY_SIMPLEX
            scale = 0.48
            thickness = 1
            text_size, baseline = cv2.getTextSize(label, font, scale, thickness)

            tx = max(2, min(x - r, w_img - text_size[0] - 2))
            ty = max(text_size[1] + 2, y - r - 6)
            box_tl = (tx - 2, ty - text_size[1] - 2)
            box_br = (tx + text_size[0] + 2, ty + baseline + 2)
            cv2.rectangle(out, box_tl, box_br, (0, 0, 0), -1)
