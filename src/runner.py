...
class RunStats:
    ...
    def update(self, diff: int, value_diff: Optional[float], true_value: Optional[float]):
        self.processed += 1
    ...
    def print_summary(self):
        ...
        count_accuracy = (self.correct_count / self.processed) * 100.0
        ...
        general_accuracy = (
            (count_accuracy + value_accuracy) / 2.0
            if self.processed_value > 0
            else count_accuracy
        )
...
class PipelineApp:
    ...
    def run(self):
        ...
        for _, row in df.iterrows():
            ...
            stats.update(diff, value_diff, true_value_num)

            status = "PERFECT" if diff == 0 else "ERROR"
            true_value_text = f"{float(true_value):.2f}" if true_value == true_value else "N/A"
            ...
            if result.coin_labels:
                label_hist = self._format_label_hist(result.coin_labels)
