...
\begin{frame}{Dataset and Evaluation Protocol}
  \begin{itemize}
    ...
    \item Metrics computed by \texttt{src/evaluation/metrics.py} and \texttt{src/evaluation/difficulty.py}.
  \end{itemize}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Value precision / recall / F1 & 91.24\% / 65.21\% / 76.06\% \\
    Value MAE & 1.961 EUR per image \\
    \bottomrule
  \end{tabular}
\end{frame}
...
\begin{frame}{Preprocessing Choices and Why}
  \begin{tabular}{@{}p{0.36\linewidth}p{0.58\linewidth}@{}}
    ...
    Normalization path in runtime & Current implementation keeps a compact preprocessing path (grayscale + blur) for stable behavior across groups. \\
    \bottomrule
  \end{tabular}
\end{frame}
...
\begin{frame}{Stage 1: Preprocessing}
  \begin{itemize}
    ...
    \item In current implementation:
    \begin{itemize}
      \item Preprocessing path kept intentionally compact: grayscale + blur.
...
\begin{frame}{Global Performance Snapshot}
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        \item High value precision (\(91.24\%\)): few large over-estimations.
        \item Good behavior on clean groups (\texttt{gp2}, \texttt{gp7}).
      \end{itemize}
      \textbf{Observed limits}
      \begin{itemize}
        \item Coin count exactness drops on dense scenes.
        \item Recall (\(65.21\%\)) affected by missed coins in difficult backgrounds.
        \item Value MAE increases strongly when many coins are missed.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Reproducibility}
  \small
  \textbf{Run full project evaluation}
  \begin{verbatim}
./.venv/bin/python main.py --no-view
  \end{verbatim}

  \textbf{Generate presentation figures from notebook}
  \begin{verbatim}
../.venv/bin/jupyter nbconvert --to notebook --execute \
  presentation/notebooks/user_selected_cases.ipynb \
  --output user_selected_cases.executed.ipynb \
  --output-dir presentation/notebooks
  \end{verbatim}
